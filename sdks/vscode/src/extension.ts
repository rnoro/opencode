...
export function activate(context: vscode.ExtensionContext) {
  ...
  const provider = new SidebarProvider(manager, diffProvider, context, context.extensionUri)
...
class SidebarProvider implements vscode.WebviewViewProvider {
  ...
  private async render(view: vscode.WebviewView) {
    ...
    try {
      ...
      const serverScript = webviewServerScript(local, root, nonce)
      ...
      const disposable = view.webview.onDidReceiveMessage((msg) =>
        handleWebviewMessage(view.webview, local, msg, this.diffProvider, this.context),
      )
...
async function handleWebviewMessage(
  ...
  switch (msg.type) {
    ...
    case "route:get": {
      const route = context.workspaceState.get<string>("opencode.lastRoute") ?? "/"
      webview.postMessage({ type: "route:init", route })
      return
    }
    case "route:set": {
      ...
      void context.workspaceState.update("opencode.lastRoute", msg.route)
      return
    }
  }
}
...
class ServerManager implements vscode.Disposable {
    ...
    return new Promise<string>((resolve, reject) => {
      void this.reset(root).finally(() => {
        ...
        proc.stdout.on("data", (chunk) => {
          ...
          this.url = match[1]
          void this.claim(root, proc.pid ?? 0, match[1])
  ...
  private async reset(root: string) {
    const paths = this.storage(root)
    if (!paths || !existsSync(paths.json)) {
      return
    }
  ...
  private storage(root: string) {
    ...
    const json = join(dir, `server.${hash}.json`)
    const lock = join(dir, `server.${hash}.lock`)
    return { json, lock }
  }

  private load(root: string): ServerRecord | undefined {
    const paths = this.storage(root)
    if (!paths || !existsSync(paths.json)) {
      return
    }
    ...
    try {
      const data = JSON.parse(readFileSync(paths.json, "utf-8")) as ServerRecord
      if (!data || typeof data !== "object") {
        return
      }
      if (typeof data.url !== "string" || typeof data.pid !== "number" || typeof data.root !== "string") {
        return
      }
      return data
    } catch {
  ...
  private save(root: string, record: ServerRecord) {
    const paths = this.storage(root)
    if (!paths) {
      return
    }
  ...
  private withLock(root: string, fn: () => void) {
    const paths = this.storage(root)
    if (!paths) {
      return
    }
  ...
  private async claim(root: string, pid: number, url: string) {
    const paths = this.storage(root)
    if (!paths || !pid) {
      return
    }
    ...
    this.withLock(root, () => {
      ...
      this.save(root, record)
    })
  }

  private release(root: string, proc?: ChildProcessWithoutNullStreams) {
    const paths = this.storage(root)
    if (!paths) {
      return
    }
    ...
    this.withLock(root, () => {
      ...
      this.terminate(proc?.pid ?? record.pid)
...
function webviewServerScript(local: string, workspace: string, nonce: string) {
  return `<script nonce="${nonce}">window.__OPENCODE_SERVER_URL__=${JSON.stringify(local)};window.__OPENCODE_WORKSPACE_KEY__=${JSON.stringify(workspace)};</script>`
}
